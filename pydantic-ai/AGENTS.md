# AGENTS.md — operating contract for ainfera-pydantic

Audience: the Ainfera fleet (Aulë and peers) and any agent operating this repo. Read before touching code.

## Identity
- **Adapter** — Pydantic AI + **Ainfera Routing** — typed agents through the gateway. One of the public framework adapters (siblings: `ainfera-langchain`, `ainfera-microsoft-agent-framework`, `ainfera-letta`, …). A *customer-integration* surface, not core infra.
...
## Definition of done — proof, not PR proof
...
pip install -e .        # this adapter ships as a package (pyproject.toml)
# then run the example in the README Quickstart
```
